	...
	<body>
		...
		<div id="show-review" class="content scaffold-show" role="main">
			...
			<ol class="property-list review">
				...
				<g:if test="${reviewInstance?.stock}">
				<li class="fieldcontain">
					<span id="stock-label" class="property-label"><g:message code="review.stock.label" default="Stock" /></span>
					
						<span class="property-value" aria-labelledby="stock-label"><g:link controller="stock" action="show" id="${reviewInstance?.stock?.id}">${reviewInstance?.stock?.encodeAsHTML()}</g:link></span>
